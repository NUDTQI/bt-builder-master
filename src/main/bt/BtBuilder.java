...
public class BtBuilder {
	...
	private final String glamProcessingFlagFolder;
	...
	public BtBuilder() throws IOException {
		...
		// added by zq 160911, create folder first
		glamProcessingFlagFolder = Util.getPropertyNotNull(p, "glam_processing_flagfolder");
	}
...
//		BehaviourTree tree = makeTree(dbi, new File(bt_filename), NodeType.CASE_BT_NODES);
		...
		for (int iteration = 1; iteration <= 50; iteration++) {
			...
			// added by zq16.09.11, solve file create in windows
			File glamProcessingfolder = new File(glamProcessingFlagFolder);
			if (!glamProcessingfolder.exists()){
				glamProcessingfolder.mkdirs();
			}
